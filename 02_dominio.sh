...
get_cloudflare_cert() {
    ...
    log $GREEN "Certificado Cloudflare instalado com sucesso."
}
...
EOF
    ...
    cat <<EOF | sudo tee /etc/nginx/sites-available/api.devcloud.top > /dev/null
server {
    listen 80;
...
EOF
    ...
    sudo ln -sf /etc/nginx/sites-available/api.devcloud.top /etc/nginx/sites-enabled/
